...
if __name__ == '__main__':
    ...
    # dataname = 'v3appendix--cont-bath'
    dataname = 'v3appendix2-cont-bath'
    ...
    for file in data_files:
        ...
        plt.imshow(depth_from_phase, cmap='gray')
        plt.colorbar()
        plt.title("depth_from_phase")
        # plt.show()
        plt.subplot(2, 1, 2)
        plt.imshow(depth_2220_nz, cmap='gray')
        plt.colorbar()
        plt.title("gt_depth")
        plt.savefig(os.path.join(npz_folder_path,file+'-'+dataname+'-phase-gt-image.png'))
        plt.close()
