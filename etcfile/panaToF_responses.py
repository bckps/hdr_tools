...
if __name__ == '__main__':
    dataname = 'bathroom-small-simu-20images'
    ...
    for file in data_files:
        ...
        accumconv = conv[height//2,width//2,:].cumsum()/conv[height//2,width//2,:].sum()

        # accumconv2220_lu = conved_img[10,110,:].cumsum()/conved_img[10,110,:].sum()
        accumconv_lu = conv[height//4,width//4,:].cumsum()/conv[height//4,width//4,:].sum()
        # accumconv2220_ll = conved_img[10,110,:].cumsum()/conved_img[10,110,:].sum()
        accumconv_ll = conv[3*height//4,width//4,:].cumsum()/conv[3*height//4,width//4,:].sum()
        # accumconv2220_c = conved_img[10,110,:].cumsum()/conved_img[10,110,:].sum()
        accumconv_c = conv[height//2,width//2,:].cumsum()/conv[height//2,width//2,:].sum()
        # accumconv2220_ru = conved_img[10,110,:].cumsum()/conved_img[10,110,:].sum()
        accumconv_ru = conv[height//4,3*width//4,:].cumsum()/conv[height//4,3*width//4,:].sum()
        # accumconv2220_rl = conved_img[10,110,:].cumsum()/conved_img[10,110,:].sum()
        accumconv_rl = conv[3*height//4,3*width//4,:].cumsum()/conv[3*height//4,3*width//4,:].sum()
        ...
        arg666 = np.abs((timeseq2960 - 66.6)).argmin()
            ...
            #####################################################################
        store_data = {'CumulativeDegree-left-upper': accumconv_lu[2220-1],
                      'CumulativeDegree-left-lower': accumconv_ll[2220 - 1],
                      'CumulativeDegree-center': accumconv_c[2220 - 1],
                      'CumulativeDegree-right-upper': accumconv_ru[2220 - 1],
                      'CumulativeDegree-right-lower': accumconv_rl[2220 - 1],
                      'CumulativeDegree-rough-average': np.mean([accumconv_lu[2220-1],accumconv_ll[2220 - 1],accumconv_c[2220 - 1],accumconv_ru[2220 - 1],accumconv_rl[2220 - 1]]),
                      'CumulativeDegree-center-99%-time': timeseq2960[arg99conv],
        ...
        # plt.figure()
        # plt.plot(timeseq,response_img[10,110,:], label="test")
        # plt.xlabel('Time [ns]')
        ...
        #
        # plt.figure()
        # plt.plot(np.linspace((30e-3),(30e-3)*2960, num=2959),conv[10,110,:], label="conv")
        # plt.xlabel('Time [ns]')
        # plt.ylabel('Pixel value')
        # plt.show()
        #
